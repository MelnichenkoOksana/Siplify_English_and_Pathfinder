...
class Point {
    ...
    static class PathNode extends Point {
        public Point goal;
    ...
    static class newSolution {
        ...
        public static void main(String[] args) throws IOException {
            matrix = InputOutput.conversionStringToArray(InputOutput.getInfoFromFile());
            ...
            InputOutput.writeFile(Integer.toString((list.size() - 1) * 5));
        ...
        public static int CountInList(ArrayList<PathNode> list, PathNode pathNode)//проверяет есть ли точка с этими координатами в closedSet
        {
            ...
            for (PathNode point : list) {
                if (point.position.Z == pathNode.position.Z && point.position.X == pathNode.position.X && point.position.Y == pathNode.position.Y) {
                    return 1;
                }
            }
            return 0;
